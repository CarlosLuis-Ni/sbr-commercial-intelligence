...
Deno.serve(async(req)=>{
  ...
  try{
    ...
    const mt=await downloadPrivate("proveedores.json");if(mt===null)return json({error:"SBR_MANIFEST_STORAGE_ERROR"},500);
    ...
    if(body.action==="provider"){
      ...
      const pt=await downloadPrivate(providerId+".json");if(pt===null)return json({error:"SBR_PROVIDER_STORAGE_ERROR"},500);
